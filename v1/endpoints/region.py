...
@router.get("/all")
async def get_regions(db: Session = Depends(dependencies.get_database_session)):
  try:
    result = db.execute('SELECT JSON_OBJECT("id", pdr.province_id, "name_np", pdr.name_np, "name_en", pdr.name_en, "color", pdr.color, "districts", pdr.districts) from ds_v_province_district_regions pdr;')
    data = []
    for p in (result):
      data.append(json.loads(p[0]))
    return {"data": {"provinces": data}, "message": "Data Read Successfully", "status": "OK"}
